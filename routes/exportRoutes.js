 ...
 */
router.get('/members/approved', authenticateJWT, async (req, res) => {
  try {
    // Find all approved membership renewals
    const approvedMembers = await MemberRenewal.find({ status: 'approved' })
      .sort({ renewalDate: -1 });
    ...
    // Convert to CSV
    const fields = ['Name', 'Email', 'Phone', 'Birthday', 'Member Since', 
                   'Renewal Date', 'Ministry Involvement', 'Address Changed', 'New Address'];
    const json2csvParser = new json2csv({ fields });
    const csv = json2csvParser.parse(membersData);
    
    // Set response headers for file download
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename=approved_members_${new Date().toISOString().slice(0, 10)}.csv`);
    
    // Send CSV data
    return res.status(200).send(csv);
  } catch (error) {
    console.error('Error exporting approved members:', error);
    return res.status(500).json({ message: 'Failed to export approved members', error: error.message });
  }
});
 ...
 * @access Private (Admin only)
 */
router.get('/foundation-classes/completed', authenticateJWT, async (req, res) => {
  try {
    // Find all completed foundation class registrations
    const completedMembers = await FoundationClassRegistration.find({ status: 'completed' })
      .sort({ updatedAt: -1 });
    
    if (completedMembers.length === 0) {
      return res.status(404).json({ message: 'No completed foundation class members found' });
    }
    ...
    // Convert to CSV
    const fields = ['Name', 'Email', 'Phone', 'Registration Date', 
                   'Completed Date', 'Preferred Session', 'Questions'];
    const json2csvParser = new json2csv({ fields });
    const csv = json2csvParser.parse(membersData);
    
    // Set response headers for file download
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename=foundation_class_graduates_${new Date().toISOString().slice(0, 10)}.csv`);
    
    // Send CSV data
    return res.status(200).send(csv);
  } catch (error) {
    console.error('Error exporting foundation class graduates:', error);
    return res.status(500).json({ message: 'Failed to export foundation class graduates', error: error.message });
  }
});
