...
const staticSermons = [
  {
    ...
    description: "Discover how faith can transform your life and overcome any obstacle in your path.",
    duration: "45:30"
  },
  {
    id: 2,
  ...
  },
  {
    ...
    videoId: "VgTVfZ3O-7A",
    speaker: "Pastor John Doe",
    description: "Understand the transformative power of prayer in your daily walk with Christ.",
    duration: "42:10"
  },
];
...
const Sermons = () => {
  ...
  const getSermonImageUrl = (sermon) => {
    if (sermon.image?.path) {
      return sermon.image.path.startsWith('/') ? `${API_URL}${sermon.image.path}` : sermon.image.path;
  ...
  useEffect(() => {
    const checkConnectivity = async () => {
      ...
      if (!canAccessYouTube) {
        console.warn('YouTube appears to be inaccessible. Videos may not play correctly.');
      }
    };
  ...
  const selectSermon = (sermon) => {
    ...
    const timeout = setTimeout(() => {
      if (isLoading) {
        ...
        setIsLoading(false);
        setVideoError(true);
        setVideoErrorMessage("Video loading timed out. Please try again or use a different player.");
      }
    ...
    if (!youtubeAccessible) {
      console.warn('YouTube appears to be inaccessible, but will try to load video anyway.');
    }
  ...
  useEffect(() => {
    const fetchSermons = async () => {
      try {
        ...
        console.log("Sermons page - API data:", data);
        setSermons(data);

        // Set the initially selected sermon
        if (videoIdParam) {
          const foundSermon = data.find(
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="bg-gray-900 text-white pt-32 pb-12">
        <div className="container mx-auto px-4">
          ...
          <p className="text-xl text-center mt-4 text-gray-300">Watch or listen to past messages from our church services</p>
        </div>
      </div>
          ...
          <div className="mb-12">
                      ...
                      <div className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center bg-gray-900 text-white p-6 text-center">
                        ...
                        {!youtubeAccessible && (
                          <div className="bg-red-800 p-3 mb-4 rounded text-sm">
                            <p>YouTube connectivity check failed. Your network may be blocking YouTube content.</p>
                            <p className="mt-2">
                              Try using a different network or check your firewall/proxy settings.
                            </p>
                          </div>
                        )}
                        ...
                        <div className="mt-4 text-xs text-gray-400">
                          <p>Debug Info: Video ID: {selectedSermon.videoId}, YouTube API Accessible: {youtubeAccessible ? 'Yes' : 'No'}</p>
                        </div>
                      </div>
                    ) : (
                      <iframe
                        ...
                        onError={(e) => {
                          ...
                          setVideoError(true);
                          setVideoErrorMessage("Failed to connect to YouTube. Check your internet connection or try another player option.");
                          setIsLoading(false);
                        }}
                      />
                    )}
                  ...
                  <>
                    {videoError ? (
                      <div className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center bg-gray-900 text-white p-6 text-center">
                        <div className="text-xl mb-4">Video Playback Error</div>
                        <p className="mb-4">
                          {videoErrorMessage || "There was an issue playing this video. Try the primary player or YouTube option."}
                        </p>
                        {!youtubeAccessible && (
                          <div className="bg-red-800 p-3 mb-4 rounded text-sm">
                            <p>YouTube connectivity check failed. Your network may be blocking YouTube content.</p>
                      ...
                      <iframe
                        ...
                        onError={(e) => {
                          ...
                          setVideoError(true);
                          setVideoErrorMessage("Failed to connect to YouTube-nocookie domain. Check your internet connection or try another player option.");
                          setIsLoading(false);
                        }}
                      />
                    )}
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-center lg:justify-start">
                <span className="mr-3 text-sm text-gray-500 dark:text-gray-400">Share:</span>
                <ShareButtons 
              ...
              {(sermon.imageUrl || sermon.image) && (
                <div className="relative h-40 overflow-hidden">
                  <img
                    ...
                    onError={(e) => {
                      console.error(`Failed to load image: ${getSermonImageUrl(sermon)}`);
