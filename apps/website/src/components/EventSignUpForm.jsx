...
const EventSignUpForm = ({ event, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    ...
    if (event.type === "baptism") {
      ...
      console.log(
        "This is a baby dedication event - dedication fields will be shown"
      );
    } else {
  ...
  const validateForm = () => {
    ...
    if (isBabyDedication) {
      ...
      if (!formData.parentNames.trim())
        errors.parentNames = "Parent names are required";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!event || !event.id) {
      setError(
        "Event information is incomplete. Unable to submit your request."
      );
    ...
    try {
      ...
      setError(
        `There was an error submitting your request: ${error.message || "Unknown error"}. Please try again.`
      );
    } finally {
          ...
          <>
            ...
            <div className="text-center py-4">
              ...
              <p className="text-gray-600 mb-6">
                Thank you for signing up for this event. We'll be in touch with
                more details soon.
              </p>
              <button
                onClick={onClose}
