...
const Sermons = () => {
  ...
  const getSermonImageUrl = (sermon) => {
    ...
    if (sermon.image?.path) {
      const url = sermon.image.path.startsWith('/') ? `${config.API_URL}${sermon.image.path}` : sermon.image.path;
      console.log('Using image.path:', url);
      return url;
    } else if (sermon.imageUrl) {
      const url = sermon.imageUrl.startsWith('/') ? `${config.API_URL}${sermon.imageUrl}` : sermon.imageUrl;
      console.log('Using imageUrl:', url);
      return url;
    } else if (typeof sermon.image === 'string') {
      const url = sermon.image.startsWith('/') ? `${config.API_URL}${sermon.image}` : sermon.image;
  ...
  useEffect(() => {
    const fetchSermons = async () => {
      try {
        ...
        console.log("API Sermon Data:", data); // Log the actual data received
        setSermons(data);
        setError(null);
      } catch (err) {
        console.error("Error fetching sermons:", err);
        setError("Failed to load sermons from API, using static data");
  ...
  if (error || !sermons || sermons.length === 0) {
    return (
      <section className="py-20 px-6 bg-gray-50">
        <div className="container mx-auto">
          <div className="flex items-center mb-12">
            <div className="w-12 h-1 bg-primary-500 mr-4"></div>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-800">Latest Message</h2>
          </div>
          <div className="p-6 bg-white rounded-xl shadow-lg">
            <p className="text-gray-700">
              {error || "No sermons available at the moment. Please check back later."}
            </p>
          </div>
        </div>
      </section>
        ...
        >
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="grid md:grid-cols-5 gap-0">
              {/* Video Thumbnail */}
              <div className="md:col-span-3 relative group cursor-pointer h-64 md:h-auto" 
              ...
              <div className="md:col-span-2 p-6 md:p-8 flex flex-col">
                <div className="flex-1">
                  <h3 className="text-2xl font-bold text-gray-800 mb-2">{latestSermon.title}</h3>
                  <p className="text-primary-600 font-medium mb-1">{latestSermon.speaker}</p>
                  <p className="text-gray-500 mb-4">{formatSermonDate(latestSermon.date)}</p>
          ...
          {sermons.slice(1, 4).map((sermon, index) => (
            ...
            >
              ...
              <div className="p-5">
                ...
                <p className="text-primary-600 text-sm mb-1">{sermon.speaker}</p>
                <p className="text-gray-500 text-sm mb-3">{formatSermonDate(sermon.date)}</p>
                <Link
                  to={`/media/sermons?video=${sermon.videoId}`}
