...
const RequestsManager = () => {
  ...
  const fetchRenewals = async () => {
    try {
      ...
      toast.error('Error loading membership renewals');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchEnrollments = async () => {
    try {
      ...
      setError('Failed to load foundation class enrollments. Please try again.');
      toast.error('Error loading foundation class enrollments');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEnrollmentStatusChange = async (id, newStatus) => {
    try {
      ...
      setError('Failed to update status. Please try again.');
      toast.error('Failed to update class enrollment status');
    }
  };
  ...
  const approveAndNotifyMember = async (member) => {
    try {
      ...
      toast.error('Status updated but failed to send notification');
    }
  };
  ...
  const approveAndSendSchedule = async (enrollee) => {
    try {
      ...
      toast.error('Status updated but failed to send class schedule');
    }
  };
  ...
  const cancelAndNotifyEnrollee = async (enrollee) => {
    try {
      ...
      toast.error('Status updated but failed to send cancellation notice');
    }
  };
  ...
  const completeAndNotifyMember = async (enrollee) => {
    try {
      ...
      toast.success(`${enrollee.fullName} has been notified of new membership status`);
      setShowEnrollmentDetails(false);
    } catch (err) {
      console.error('Error completing and notifying new member:', err);
      toast.error('Status updated but failed to send completion notice');
    }
  };
  ...
  const downloadMembersList = async () => {
    try {
      const approvedMembers = renewals.filter(renewal => renewal.status === 'approved');
      ...
      link.href = url;
      link.setAttribute('download', `approved_members_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const downloadFoundationGraduatesList = async () => {
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `foundation_class_graduates_${new Date().toISOString().slice(0, 10)}.csv`);
      document.body.appendChild(link);
      ...
      toast.error('Failed to download graduates list');
    }
  };
  ...
  // Loading indicator
  if (loading && ((activeTab === 'membership' && renewals.length === 0) || 
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={activeTab === 'membership' ? fetchRenewals : fetchEnrollments}
          ...
          <div className="flex flex-col md:flex-row gap-4 md:items-center">
            ...
            {activeTab === 'membership' && (
              <button
                ...
                className="w-full md:w-auto flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
              >
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {sortedEnrollments.map((enrollment) => (
                    <tr key={enrollment.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            ...
                            <div className="text-sm text-gray-500">Registered: {formatDate(enrollment.registrationDate)}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeClasses(enrollment.status)}`}>
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
              <h3 className="text-lg font-medium text-gray-900">Membership Renewal Details</h3>
              <button
            ...
            <div className="px-6 py-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    <p className="flex items-start"><span className="font-medium w-32">Name:</span> {selectedRenewal.fullName}</p>
                    <p className="flex items-start"><span className="font-medium w-32">Email:</span> {selectedRenewal.email}</p>
                    <p className="flex items-start"><span className="font-medium w-32">Phone:</span> {selectedRenewal.phone}</p>
                    <p className="flex items-start"><span className="font-medium w-32">Member Since:</span> {selectedRenewal.memberSince}</p>
                    <p className="flex items-start"><span className="font-medium w-32">Birthday:</span> {formatDate(selectedRenewal.birthday)}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    <p className="flex items-start"><span className="font-medium w-32">Renewal Date:</span> {formatDate(selectedRenewal.renewalDate)}</p>
                    <p className="flex items-start"><span className="font-medium w-32">Status:</span> 
                    ...
                    <p className="flex items-start"><span className="font-medium w-32">Address Change:</span> {selectedRenewal.addressChange ? 'Yes' : 'No'}</p>
                    {selectedRenewal.addressChange && selectedRenewal.newAddress && (
                      <p className="flex items-start"><span className="font-medium w-32">New Address:</span> {selectedRenewal.newAddress}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {selectedRenewal.ministryInvolvement && (
                <div className="mb-6">
                  ...
                  <p className="mt-2 text-sm text-gray-600">{selectedRenewal.ministryInvolvement}</p>
                </div>
              )}
      ...
      {showEnrollmentDetails && selectedEnrollment && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
              <h3 className="text-lg font-medium text-gray-900">Foundation Class Enrollment Details</h3>
              <button
            ...
            <div className="px-6 py-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                  <h4 className="text-sm font-medium text-gray-500">Enrollee Information</h4>
                  <div className="mt-2 space-y-2">
                    <p className="flex items-start"><span className="font-medium w-32">Name:</span> {selectedEnrollment.fullName}</p>
                    <p className="flex items-start"><span className="font-medium w-32">Email:</span> {selectedEnrollment.email}</p>
                    <p className="flex items-start"><span className="font-medium w-32">Phone:</span> {selectedEnrollment.phone}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    <p className="flex items-start"><span className="font-medium w-32">Registration Date:</span> {formatDate(selectedEnrollment.registrationDate)}</p>
                    <p className="flex items-start"><span className="font-medium w-32">Preferred Session:</span> {selectedEnrollment.preferredSession}</p>
                    <p className="flex items-start"><span className="font-medium w-32">Status:</span> 
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadgeClasses(selectedEnrollment.status)}`}>
              ...
              {selectedEnrollment.questions && (
                <div className="mb-6">
                  <h4 className="text-sm font-medium text-gray-500">Questions/Comments</h4>
                  <p className="mt-2 text-sm text-gray-600">{selectedEnrollment.questions}</p>
                </div>
              )}
