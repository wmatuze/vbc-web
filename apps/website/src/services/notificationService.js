...
class NotificationService {
  ...
  static async sendEventSignupApprovalNotification(request) {
    try {
      ...
      const response = await axios.post(
        `${getApiUrl()}/api/notifications/send`,
        {
          ...
          data: {
            ...
            eventDate:     request.eventId?.date     || "",
            eventTime:     request.eventId?.time     || "",
      ...
      console.error("Failed to send event signup approval notification:", error.response?.data || error.message);
      throw error;
    }
  }
