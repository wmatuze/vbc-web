    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.upload.addEventListener('progress', (event) => {
        if (event.lengthComputable && onProgress) {
          ...
          onProgress({ loaded: event.loaded, total: event.total, percent: progressPercent });
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            ...
            if (!response.path) {
              ...
              reject(new Error('Server response missing required path property'));
              return;
            }
      ...
      xhr.ontimeout = function() {
        ...
        reject(new Error('Upload request timed out. The server may be busy or unavailable.'));
      };
...
export const updateSermon = (id, sermon) => updateData('api/sermons', id, sermon);
...
export const updateLeader = (id, leader) => updateData('api/leaders', id, leader);
...
export const updateCellGroup = (id, group) => updateData('api/cell-groups', id, group);
  ...
  try {
    ...
    try {
      ...
      if (sessionCache) {
        ...
        console.log(`Recovered ${sessionData.length} media items from session cache`);
        return sessionData;
      }
    } catch (sessionError) {
      console.error('Could not recover media from session cache:', sessionError);
    }
    ...
    try {
      ...
      if (localBackup) {
        ...
        console.log(`Recovered ${backupData.length} media items from local backup`);
...
export const login = async (username, password) => {
  ...
  for (const credentialMode of credentialModes) {
    for (const corsMode of corsModes) {
      try {
        console.log(`Trying login with credentials: ${credentialMode}, mode: ${corsMode}`);
